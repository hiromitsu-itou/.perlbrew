...
documentation in L<HTML::FormHandler::Field#Attributes_for_creating_HTML>.
...
Use a widget_wrapper of 'None', and set widget_tag 'no_auto_fieldset' to 1:
    ...
    has '+widget_tags' => ( default => sub { { no_auto_fieldset => 1 } } );
    has_field 'foo';
    has_field 'bar';
...
that it uses; you can add your own for your custom rendering code.
    ...
    label_no_colon -- don't put a ': ' following the label
    no_auto_fieldset -- don't put a fieldset around main form
    no_compound_wrapper -- don't put a fieldset wrapper around compound fields
...
label formatting:
   ...
   has_field 'my_field' => (
       widget_tags => {wrapper_tag => 'p', label_no_colon => 1  } );
...
form/field objects or the L<HTML::FormHandler::Result> objects. For that reason,
