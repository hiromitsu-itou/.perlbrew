...
#endif
    if (items < 2 || items > 6)
       croak_xs_usage(cv,  "isHASH, dbtype, name=undef, flags=O_CREAT|O_RDWR, mode=0666, type=DB_HASH");
    {
...
#endif
    if (items < 2 || items > 3)
       croak_xs_usage(cv,  "db, key, flags=0");
    {
	DB_File	db;
	DBTKEY	key;
	u_int	flags;
...
#line 1795 "DB_File.c"
	...
	if (SvROK(ST(0)) && sv_derived_from(ST(0), "DB_File")) {
	    IV tmp = SvIV((SV*)SvRV(ST(0)));
	    db = INT2PTR(DB_File,tmp);
	}
	else
...
#endif
    if (items < 3 || items > 4)
       croak_xs_usage(cv,  "db, key, value, flags=0");
    {
	DB_File	db;
	DBTKEY	key;
	DBT	value;
...
#endif
    if (items != 4)
       croak_xs_usage(cv,  "db, key, value, flags");
    {
	DB_File	db;
	DBTKEY	key;
	DBT	value;
...
XS(boot_DB_File)
{
...

    {
        ...
        (void)newXS("DB_File::constant", XS_DB_File_constant, file);
        (void)newXS("DB_File::DoTie_", XS_DB_File_DoTie_, file);
        (void)newXS("DB_File::DESTROY", XS_DB_File_DESTROY, file);
        (void)newXS("DB_File::DELETE", XS_DB_File_DELETE, file);
        (void)newXS("DB_File::EXISTS", XS_DB_File_EXISTS, file);
        (void)newXS("DB_File::FETCH", XS_DB_File_FETCH, file);
        (void)newXS("DB_File::STORE", XS_DB_File_STORE, file);
        (void)newXS("DB_File::FIRSTKEY", XS_DB_File_FIRSTKEY, file);
        (void)newXS("DB_File::NEXTKEY", XS_DB_File_NEXTKEY, file);
        ...
        XSANY.any_i32 = 1 ;
        (void)newXS("DB_File::del", XS_DB_File_del, file);
        (void)newXS("DB_File::get", XS_DB_File_get, file);
        (void)newXS("DB_File::put", XS_DB_File_put, file);
        (void)newXS("DB_File::fd", XS_DB_File_fd, file);
        (void)newXS("DB_File::sync", XS_DB_File_sync, file);
        (void)newXS("DB_File::seq", XS_DB_File_seq, file);
        (void)newXS("DB_File::filter_fetch_key", XS_DB_File_filter_fetch_key, file);
        (void)newXS("DB_File::filter_store_key", XS_DB_File_filter_store_key, file);
        (void)newXS("DB_File::filter_fetch_value", XS_DB_File_filter_fetch_value, file);
        (void)newXS("DB_File::filter_store_value", XS_DB_File_filter_store_value, file);
    }
